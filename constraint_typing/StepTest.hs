...
stepTests = TestList
    [
      ...
      step [(v_x, ty_X)] f t_x ~?= Right (ty_X, f, [])
    , "項が変数のみからなり、その変数が型付け文脈に含まれない場合、型付けが失敗する。" ~:
      step [] f t_x ~?= Left "error"

    , "項がzeroの場合、Nat型に型付けされる。" ~:
      step [] f TmZero ~?= Right (TyNat, f, [])

    , "項がsucc zeroの場合、Nat型に型付けされる。" ~:
      step [] f (TmSucc TmZero) ~?= Right (TyNat, f, [])
    , "項がsucc succ zeroの場合、Nat型に型付けされる。" ~:
      step [] f (TmSucc (TmSucc TmZero)) ~?= Right (TyNat, f, [])
    , "項がsucc trueの場合、型付けが失敗する。" ~:
      step [] f (TmSucc TmTrue) ~?= Left "error"
    , "項がsucc xで、xが型付け文脈でNatに型付けされている場合、Natに型付けされる。" ~:
      step [(v_x, TyNat)] f (TmSucc t_x) ~?= Right (TyNat, f, [])
    , "項がsucc xで、xが型付け文脈でBoolに型付けされている場合、型付けが失敗する。" ~:
      step [(v_x, TyBool)] f (TmSucc t_x) ~?= Left "error"
    , "項がsucc xで、xが型付け文脈に現れない場合、型付けが失敗する。" ~:
      step [(v_x, TyBool)] f (TmSucc t_x) ~?= Left "error"

    , "項がpred zeroの場合、Nat型に型付けされる。" ~:
      step [] f (TmPred TmZero) ~?= Right (TyNat, f, [])
    , "項がpred trueの場合、型付けが失敗する。" ~:
      step [] f (TmPred TmTrue) ~?= Left "error"

    , "項がiszero succ zeroの場合、型付けが失敗する。" ~:
      step [] f (TmIsZero (TmSucc TmZero)) ~?= Right (TyNat, f, [])
    , "項がiszero trueの場合、型付けが失敗する。" ~:
      step [] f (TmIsZero TmTrue) ~?= Left "error"

    , "項がtrueの場合、Bool型に型付けされる。" ~:
      step [] f TmTrue ~?= Right (TyBool, f, [])

    , "項がfalseの場合、Bool型に型付けされる。" ~:
      step [] f TmFalse ~?= Right (TyBool, f, [])
